...
def implicit_conv2d(
            ...
            for rest_n in cutlass.range_constexpr(tApA.shape[2]):
                for rest_k in cutlass.range_constexpr(cute.size(tApA, [3])):
                    ...
                    tApA[frg_x, rest_m, rest_n, rest_k] = \
                        p < v_act.shape[0][1] and q < v_act.shape[0][2] and \
    ...
    cute.copy(s2r_tiled_copy_A, tCsA_copy[None, None, 0, 0], tCrA_copy[None, None, 0])
    cute.copy(s2r_tiled_copy_B, tCsB_copy[None, None, 0, 0], tCrB_copy[None, None, 0])
    ...
    for k_iter in range(K_ITERS):
        ...
        if k_iter != K_ITERS - 1:
            cute.arch.cp_async_wait_group(S_PIPES - 2)
            cute.arch.sync_threads()
            cute.copy(s2r_tiled_copy_A, tCsA_copy[None, None, 0, pipe_r], tCrA_copy[None, None, 0])
            cute.copy(s2r_tiled_copy_B, tCsB_copy[None, None, 0, pipe_r], tCrB_copy[None, None, 0])
        tile_idx += 1
...
def entry(
    ...
    sA_layout = cute.make_composed_layout(
        inner=cute.make_swizzle(b=1, m=3, s=3),
        offset=0,
    ...
    sB_layout = cute.make_composed_layout(
        inner=cute.make_swizzle(b=2, m=3, s=4),
        offset=0,
        outer=cute.make_ordered_layout(
